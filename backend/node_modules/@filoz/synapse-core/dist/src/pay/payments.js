import { isAddressEqual, maxUint256, parseSignature, } from 'viem';
...
export async function depositAndApprove(client, options) {
    ...
    const isCustomOperator = options.operator !== undefined && !isAddressEqual(options.operator, chain.contracts.fwss.address);
    if (isCustomOperator) {
        if (options.rateAllowance === undefined ||
