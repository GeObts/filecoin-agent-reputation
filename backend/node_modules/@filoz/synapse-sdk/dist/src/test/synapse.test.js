...
import { createWalletClient, isAddressEqual, parseUnits, http as viemHttp } from 'viem';
...
describe('Synapse', () => {
    ...
    describe('getProviderInfo', () => {
        it('should get provider info for valid approved provider', async () => {
            ...
            assert.ok(isAddressEqual(providerInfo.serviceProvider, Mocks.ADDRESSES.serviceProvider1));
    ...
    describe('download', () => {
        ...
        it('should accept valid PieceCID string', async () => {
            ...
            server.use(Mocks.JSONRPC(Mocks.presets.basic), http.get('https://pdp.example.com/pdp/piece', async ({ request }) => {
        ...
        it('should pass withCDN option to retriever', async () => {
            ...
            const testData = new TextEncoder().encode('test data');
            server.use(Mocks.JSONRPC({ ...Mocks.presets.basic }), http.head(`https://:wallet.calibration.filbeam.io/:cid`, async () => {
                return HttpResponse.text('ok', { status: 200 });
            }), http.get(`https://:wallet.calibration.filbeam.io/:cid`, async ({ params }) => {
            ...
            assert.ok(isAddressEqual(wallet, Mocks.ADDRESSES.client1));
            const data = await synapse.storage.download({ pieceCid: testPieceCid });
            assert.isTrue(data instanceof Uint8Array);
            assert.equal(new TextDecoder().decode(data), 'test data');
        });
        ...
        it('should handle download errors', async () => {
            server.use(Mocks.JSONRPC(Mocks.presets.basic), http.get('https://pdp.example.com/pdp/piece', async () => {
    ...
    describe('createContexts', () => {
        ...
        it('selects specified providerIds', async () => {
            ...
            assert.equal(contexts[0]._dataSetId, undefined);
            assert.equal(contexts[1]._dataSetId, undefined);
        });
        it('uses existing data set specified by providerId when metadata matches', async () => {
            ...
            assert.equal(BigInt(contexts[0].provider.id), Mocks.PROVIDERS.provider1.providerId);
            assert.equal(contexts[0]._dataSetId, 1n);
        });
        ...
        it('selects providers specified by data set id', async () => {
            ...
            assert.equal(contexts1[0]._dataSetId, 1n);
        });
        ...
        providerIds.forEach((endorsedProviderId, index) => {
            describe(`when endorsing providers[${index}]`, async () => {
                ...
                it('throws when endorsed provider fails ping (no fallback to non-endorsed)', async () => {
                    ...
                    server.use(http.get(`${endorsedProvider.products[0].offering.serviceURL}/pdp/ping`, () => HttpResponse.error()));
                    try {
                        await synapse.storage.createContexts({
                            count: 1,
                        });
        ...
        describe('upload', () => {
            ...
            beforeEach(async () => {
                ...
                for (const provider of [Mocks.PROVIDERS.provider1, Mocks.PROVIDERS.provider2]) {
                    ...
                    server.use(Mocks.pdp.dataSetCreationStatusHandler(FAKE_TX_HASH, {
                        ok: true,
            ...
            it('succeeds for ArrayBuffer data when upload found', async () => {
                ...
                for (const provider of [Mocks.PROVIDERS.provider1, Mocks.PROVIDERS.provider2]) {
                    ...
                    server.use(Mocks.pdp.pieceAdditionStatusHandler(DATA_SET_ID, FAKE_TX_HASH, {
                        txHash: FAKE_TX_HASH,
                        txStatus: 'confirmed',
                        dataSetId: DATA_SET_ID,
                        pieceCount: 1,
            ...
            it('fails when primary store fails', async () => {
                ...
                server.use(http.post(`${pdpOptions.baseUrl}/pdp/piece/uploads`, async () => {
                    return HttpResponse.error();
                }));
                try {
